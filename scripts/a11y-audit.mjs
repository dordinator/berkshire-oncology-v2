...
async function main() {
  ...
  const results = [];
  let checked = 0;
  // Opt-in bounded concurrency: each width/motion pair keeps an isolated
  // browser context, and report order stays deterministic after the sweep.
  const jobs = MOTION.flatMap((motion) => VIEWPORTS.map((vp) => ({ motion, vp })));
  const workers = Math.max(1, Math.min(4, Number.parseInt(args.workers || "1", 10) || 1));
  let nextJob = 0;
  ...
  results.sort((a, b) =>
    ...
    Number(a.viewport) - Number(b.viewport) || a.route.localeCompare(b.route),
  );
  ...
  const failedChecks = results.filter((r) => r.error || r.status < 200 || r.status >= 400);
  const expectedChecks = routes.length * VIEWPORTS.length * MOTION.length;
  console.log(`${results.length}/${expectedChecks} checks recorded; ${failedChecks.length} execution or HTTP failures`);
  console.log(`report: ${path.relative(ROOT, mdPath)}`);
  process.exitCode = total > 0 || failedChecks.length > 0 || results.length !== expectedChecks ? 1 : 0;
}
...
function report(results, stamp, routeCount) {
  ...
  const badStatus = results.filter((r) => !r.error && (r.status < 200 || r.status >= 400));
  ...
  lines.push(
    totalEls === 0
      ? errors.length || badStatus.length
        ? "**No violations reported by completed checks; coverage is incomplete because checks failed.**"
        : "**No automated violations detected. This is not a full WCAG conformance assessment.**"
  ...
  lines.push("");
  lines.push(
    "Automated testing cannot establish WCAG conformance. This report is one " +
      "input to an assessment; the status of manual keyboard, " +
      "zoom, reflow and screen-reader checks is recorded separately in " +
